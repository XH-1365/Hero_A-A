...
DJI_Motor_Data_Struct DJI_MGRP1_RX_Data[DJI_MGRP1_RX_SUM] = {0};
DJI_Motor_Data_Struct DJI_MGRP2_RX_Data[DJI_MGRP2_RX_SUM] = {0};
DJI_Motor_Data_Struct DJI_MGRP3_RX_Data[DJI_MGRP3_RX_SUM] = {0};
DJI_Motor_Group_Struct Motor_Group[DJI_MGRP_SUM] = {
    // 从左到右成员顺序为:Flag--Count--hcan--TX_STD_ID--RX_STD_ID--Data
    {0, 0, &hcan1, {0x200, 0x1FF}, 0x200, DJI_MGRP1_RX_Data, DJI_MGRP1_RX_SUM},
    {0, 0, &hcan1, {0x1FF, 0x2FF}, 0x204, DJI_MGRP2_RX_Data, DJI_MGRP2_RX_SUM},
    {0, 0, &hcan2, {0x200, 0x1FF}, 0x200, DJI_MGRP3_RX_Data, DJI_MGRP3_RX_SUM},
};
...
void DJI_Motor_Daemon_Init(void)
{
    Motor_Group[DJI_MGRP1].Data[DJI_MGRP1_M3508_RX_1].Daemon = Daemon_Register(DAE_CHASSIS_M3508_1, 10, NULL, ENABLE);
    Motor_Group[DJI_MGRP1].Data[DJI_MGRP1_M3508_RX_2].Daemon = Daemon_Register(DAE_CHASSIS_M3508_2, 10, NULL, ENABLE);
    Motor_Group[DJI_MGRP1].Data[DJI_MGRP1_M3508_RX_3].Daemon = Daemon_Register(DAE_CHASSIS_M3508_3, 10, NULL, ENABLE);
    Motor_Group[DJI_MGRP1].Data[DJI_MGRP1_M3508_RX_4].Daemon = Daemon_Register(DAE_CHASSIS_M3508_4, 10, NULL, ENABLE);

    Motor_Group[DJI_MGRP2].Data[DJI_MGRP2_GM6020_RX_5].Daemon = Daemon_Register(DAE_GIMBAL_GM6020_YAW, 10, NULL, ENABLE);
    Motor_Group[DJI_MGRP2].Data[DJI_MGRP2_M2006_RX_6].Daemon = Daemon_Register(DAE_GIMBAL_M2006_PITCH, 10, NULL, ENABLE);

    Motor_Group[DJI_MGRP3].Data[DJI_MGRP3_M3508_RX_1].Daemon = Daemon_Register(DAE_SHOOT_M3508_1, 10, NULL, ENABLE);
    Motor_Group[DJI_MGRP3].Data[DJI_MGRP3_M3508_RX_2].Daemon = Daemon_Register(DAE_SHOOT_M3508_2, 10, NULL, ENABLE);
}
...
void DJI_Motor_RX_Filter_Set(void)
{
    ...
    Bsp_CAN_RX_Filter_Struct Bsp_CAN_RX_Filter_config2;

    // 过滤底盘电机和云台电机，ID：0x201~0x20A
    ...
    Bsp_CAN_RX_Filter_config.FilterBank = CAN1_FILTER_ID_CHASSIS_GIMBAL;            // 过滤器编号  CAN过滤器有很多个选择其中一个即可
    ...
    // 过滤发射器电机，ID：0x201~0x202
    // HAL_Delay(1000);
    // 过滤器匹配ID设置
    Bsp_CAN_RX_Filter_config2.Filter_ID.Sub.STID = 0x200;
    Bsp_CAN_RX_Filter_config2.Filter_ID.Sub.EXID = 0x0000;
    Bsp_CAN_RX_Filter_config2.Filter_ID.Sub.IDE = 0; // 为标准帧
    Bsp_CAN_RX_Filter_config2.Filter_ID.Sub.RTR = 0; // 为数据帧

    // 掩码 ，掩码位为1时需要进行比较，为0时则可以时任意值，
    Bsp_CAN_RX_Filter_config2.Filter_Mask_ID.Sub.STID = 0x7FC;
    Bsp_CAN_RX_Filter_config2.Filter_Mask_ID.Sub.EXID = 0x0000;
    Bsp_CAN_RX_Filter_config2.Filter_Mask_ID.Sub.IDE = 1; // 必须为标准帧
    Bsp_CAN_RX_Filter_config2.Filter_Mask_ID.Sub.RTR = 1; // 必须为数据帧

    Bsp_CAN_RX_Filter_config2.FilterBank = CAN2_FILTER_ID_SHOOT;           // 过滤器编号  CAN过滤器有很多个选择其中一个即可
    Bsp_CAN_RX_Filter_config2.SlaveStartFilterBank = 14; // 起始过滤器编号应该为14，这样的话 can1(0-13)和can2(14-27)就能分别得到一半的filter
    Bsp_CAN_RX_Filter_config2.hcan = &hcan2;             // 选择CAN1或者CAN2
    Bsp_CAN_RX_Filter_config2.fifox = CAN_FilterFIFO1;
    Bsp_CAN_RX_Filter_config2.FilterActivation = CAN_FILTER_ENABLE;
    Bsp_CAN_RX_Filter_Set(&Bsp_CAN_RX_Filter_config2);
}
...
float DJI_Motor_Get_Angle_Sum(DJI_Motor_Angle_Sum_Struct *Angle_Sum_Process, float Angle_Value, float Max, float Min)
{
    ...
    if ((Angle_Sum_Process->Angle_Last - Angle_Sum_Process->Angle_Now) > (Jump_Value - 0.4f))
    {
        (Angle_Sum_Process->Angle_Turn) += Jump_Value;
    }
    else if ((Angle_Sum_Process->Angle_Last - Angle_Sum_Process->Angle_Now) < -(Jump_Value - 0.4f))
    {
        (Angle_Sum_Process->Angle_Turn) -= Jump_Value;
    }
...
char DJI_Motor_Get_Data(DJI_Motor_Group_enum DJI_MGRPx, CAN_RxHeaderTypeDef *RxHeader, uint8_t *Motor_RX_Buffer)
{
    ...
    Daemon_Reload(Motor_Group[DJI_MGRPx].Data[RX_num].Daemon);
